...
```{r load-counts}
year_counts <- read_table('year_counts.tsv', col_names = c('term','year','volume','book_count'))
...
year_plus_total <- left_join(year_counts, total_counts, by = "year") %>% mutate(mention = volume/total_volume)
...
year_plus_total %>% filter(term == 1883 | term == 1910 | term == 1950, year >= 1850 & year <= 2012) %>% mutate(factor_term = as.character(term)) %>% ggplot(aes(x = year, y = mention, color = factor_term)) + geom_line() + scale_y_continuous(label = percent) + xlab("Year") + ylab("Frequency")
...
year_plus_total %>% filter(term == 1883 | term == 1910 | term == 1950, year >= 1850 & year <= 2012) %>% mutate(factor_term = as.character(term)) %>% ggplot(aes(x = year, y = volume , color = factor_term)) + geom_line() + scale_y_continuous(label = comma) + xlab("Year") + ylab("Volume")
...
```{r plot-totals}
year_plus_total %>% filter(year >= 1850 & year <= 2012) %>% ggplot(aes(x = year, y = total_volume)) + geom_line() + scale_y_continuous(label = comma) + xlab("Year") + ylab("Volume")
...
The second visualization makes me reevaluate Michel's result. Eventhough there is a decline in the mention of terms after they've reached their peak years, we can also notice that there is a rise in those terms in the latter years. Some of the terms even rising higher than their previous peak years and it looks like or we can predict that they're continuing to rise up. 
...
peak_year <- year_plus_total %>% filter(year >= 1850 & year <= 2012) %>% group_by(term) %>% select(term, year, mention) %>% arrange(term, desc(mention))  %>% top_n(n=1) 
```
...
half_year <- left_join(peak_year, year_plus_total, by="term")


half_year %>% filter(year.y > year.x) %>% select(term,year.x,mention.x, year.y) %>% mutate(half_mention = mention.x * 0.5) %>% group_by(term) %>% arrange(term, desc(half_mention)) %>% mutate(half = year.y - year.x) %>% top_n(n=1)
...
```{r plot-half-lifes}
half_year %>% filter(year.x >= 1850 & year.x <= 2012) %>% ggplot(aes(x=year.x, y = half)) + geom_line + xlab("Year") + ylab("Half-life")
